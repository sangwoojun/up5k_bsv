 ...
 *     #include "vpi_wrapper_memRead.h"
 *     void (*vlog_startup_routines[])() = { memRead_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_memRead call=memRead_calltf size=32 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_memRead vpiSysFuncSized 32 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
unsigned int memRead(unsigned int , unsigned int );
...
PLI_INT32 memRead_calltf(PLI_BYTE8 *user_data)
{
  ...
  vpi_result = memRead(arg_1, arg_2);
...
/* sft: $imported_memRead vpiSysFuncSized 32 unsigned */

/* tab: $imported_memRead call=memRead_calltf size=32 acc=rw:%TASK */

PLI_INT32 memRead_sizetf(PLI_BYTE8 *user_data)
{
  return 32u;
}
...
void memRead_vpi_register()
{
  ...
  tf_data.tfname = "$imported_memRead";
  tf_data.calltf = memRead_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = memRead_sizetf;
  tf_data.user_data = "$imported_memRead";
